...
class PairRddDemokt {
    ...
    fun run() {
        val list = Arrays.asList<Int>(1, 2, 3, 3, 2, 1, 6, 6, 3)
        val rdd = sc.parallelize(list)
        val pairRDD = rdd.mapToPair<Int, Int> { o: Int -> Tuple2(o, 1) }

        val pairRDD2 = rdd.mapToPair<Int, Int> { o: Int -> Tuple2(o, 2) }
        //单个rdd转换操作
        ...
        //两个rdd装换操作
        //key交集的剔除
        eachPrint(pairRDD.subtractByKey<Int>(pairRDD2))
        //key一样的笛卡尔乘积还有righOuterJoin和leftOuterjoin
        eachPrint(pairRDD.join<Int>(pairRDD2))
        //lookUp取出某一个特定key的value集返回一个list
        eachPrint(sc.parallelize<Int>(pairRDD.lookup(1)))
        //数据分区，良好的数据分区对join bykey等运算是非常又用的，可以直接在一台机子上完成运算
        //减少了网络传输
        //下面是根据hash分区最多分100个区的写法，注意分区完了要持久化，否则下次还是原样的分布
        pairRDD.partitionBy(HashPartitioner(100)).persist(StorageLevel.MEMORY_AND_DISK())
    ...
    fun groupBykey(pairRDD: JavaPairRDD<*, *>): JavaPairRDD<*, *> {
        return pairRDD.groupByKey()
    }

    //reduceByKey把同一个key的value进行聚合成一个值，返回缩小版的pairRdd
    fun reduceByKey(pairRDD: JavaPairRDD<Int, Int>): JavaPairRDD<*, *> {
        return pairRDD.reduceByKey { o: Int, o2: Int -> o + o2 }
    }

    //combineByKey把同一个key的value拿出来进行分析(求均值)
    fun combineByKey(pairRDD: JavaPairRDD<Int, Int>): JavaPairRDD<*, *> {
        return pairRDD.combineByKey(
                //注意三个参数都是对相同的key的操作
                //第一个参数，如果这个key是初次出现则返回一个 value:1的初值tuple
                { o: Int -> Tuple2<Int, Int>(o, 1) },
    ...
    fun mapValues(pairRDD: JavaPairRDD<Int, Int>): JavaPairRDD<*, *> {
        return pairRDD.mapValues { o: Int -> o.toString() + "st" }
    }

    //flatMapValues只对value进行的flatmap运算返回扩大版pairRdd
    fun flatMapValues(pairRDD: JavaPairRDD<*, *>): JavaPairRDD<*, *> {
        return pairRDD.flatMapValues { o: Any -> Arrays.asList(o.toString() + "st", "多个") }
    }
